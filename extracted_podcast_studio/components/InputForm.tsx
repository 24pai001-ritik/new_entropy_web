...
export const InputForm: React.FC<InputFormProps> = ({ settings, setSettings, onGenerate, isLoading }) => {
    ...
    const handleFirstImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            const reader = new FileReader();
            reader.onloadend = () => { handleChange('firstImage', reader.result as string); };
            reader.readAsDataURL(file);
        }
    };

    const handleLastImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (file) {
            const reader = new FileReader();
            reader.onloadend = () => { handleChange('lastImage', reader.result as string); };
            reader.readAsDataURL(file);
        }
    };

    const handleColorChange = (key: keyof PodcastSettings['brandColors'], val: string) => {
        setSettings(prev => ({
            ...prev,
            brandColors: { ...prev.brandColors, [key]: val }
        }));
    };
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className={`glass-panel rounded-3xl p-8 relative overflow-hidden group transition-all ${isYoutubeDetected ? 'border-red-500/30 shadow-[0_0_30px_rgba(239,68,68,0.1)]' : ''}`}>
                        ...
                        <div className="relative space-y-4">
                            <textarea
                                className={`w-full h-32 bg-[#0B0D14] border rounded-2xl p-6 text-lg text-slate-200 placeholder-slate-600 focus:ring-1 transition-all resize-none shadow-inner ${isYoutubeDetected
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                                            {analysisModes.map((mode) => (
                                                <button
                                                    ...
                                                    className={`flex flex-col items-center p-2 rounded-lg border transition-all ${settings.youtubeAnalysisMode === mode.id
                            ...
                            <div className="grid grid-cols-2 gap-3 flex-1">
                                {CONTENT_FORMATS.map((fmt) => (
                                    <button
                                        ...
                                        className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all text-center ${settings.contentFormat === fmt.id
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="glass-panel rounded-3xl p-6 space-y-6 sticky top-24">
                            ...
                            <div className="flex bg-[#0B0D14] rounded-xl p-1 border border-slate-800">
                                {ASPECT_RATIOS.slice(0, 3).map(r => (
                                    <button
                                        ...
                                        className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${settings.aspectRatio === r ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'
                                            }`}
                                    >
                                        {r}
                                    </button>
                        ...
                        <div className="pt-2 space-y-4">
                            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Brand Assets</h3>

                            {/* Logo Upload */}
                            <div className="relative group cursor-pointer">
                                <input type="file" accept="image/*" onChange={handleLogoUpload} className="hidden" id="logo-upload" />
                                <label htmlFor="logo-upload" className="flex items-center justify-center gap-2 w-full bg-[#0B0D14] border border-slate-800 border-dashed rounded-xl px-4 py-2.5 text-slate-500 hover:text-white hover:border-purple-500 transition-all">
                                    {settings.logo ? (
                                        <div className="flex items-center gap-2 w-full justify-between">
                                            <div className="flex items-center gap-2">
                                                <img src={settings.logo} className="w-5 h-5 rounded object-cover" />
                                                <span className="text-[10px] font-bold text-purple-400">Logo Set</span>
                                            </div>
                                            <button onClick={(e) => { e.preventDefault(); handleChange('logo', null) }} className="text-slate-500 hover:text-red-400">×</button>
                                        </div>
                                    ) : (
                                        <>
                                            <Upload size={12} /> <span className="text-[10px] font-bold">Brand Logo</span>
                                        </>
                                    )}
                                </label>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {/* First Image Upload */}
                                <div className="relative group cursor-pointer">
                                    <input type="file" accept="image/*" onChange={handleFirstImageUpload} className="hidden" id="first-image-upload" />
                                    <label htmlFor="first-image-upload" className="flex items-center justify-center gap-2 w-full bg-[#0B0D14] border border-slate-800 border-dashed rounded-xl px-4 py-2.5 text-slate-500 hover:text-white hover:border-blue-500 transition-all">
                                        {settings.firstImage ? (
                                            <div className="flex items-center gap-2 w-full justify-between">
                                                <div className="flex items-center gap-2">
                                                    <img src={settings.firstImage} className="w-5 h-5 rounded object-cover" />
                                                    <span className="text-[10px] font-bold text-blue-400">Hook Set</span>
                                                </div>
                                                <button onClick={(e) => { e.preventDefault(); handleChange('firstImage', null) }} className="text-slate-500 hover:text-red-400">×</button>
                                            </div>
                                        ) : (
                                            <>
                                                <ImageIcon size={12} /> <span className="text-[10px] font-bold">First (Hook)</span>
                                            </>
                                        )}
                                    </label>
                                </div>
                                ...
                                {/* Last Image Upload */}
                                <div className="relative group cursor-pointer">
                                    <input type="file" accept="image/*" onChange={handleLastImageUpload} className="hidden" id="last-image-upload" />
                                    <label htmlFor="last-image-upload" className="flex items-center justify-center gap-2 w-full bg-[#0B0D14] border border-slate-800 border-dashed rounded-xl px-4 py-2.5 text-slate-500 hover:text-white hover:border-emerald-500 transition-all">
                                        {settings.lastImage ? (
                                            <div className="flex items-center gap-2 w-full justify-between">
                                                <div className="flex items-center gap-2">
                                                    <img src={settings.lastImage} className="w-5 h-5 rounded object-cover" />
                                                    <span className="text-[10px] font-bold text-emerald-400">End Set</span>
                                                </div>
                                                <button onClick={(e) => { e.preventDefault(); handleChange('lastImage', null) }} className="text-slate-500 hover:text-red-400">×</button>
                                            </div>
                                        ) : (
                                            <>
                                                <ImageIcon size={12} /> <span className="text-[10px] font-bold">Last (Closure)</span>
                                            </>
                                        )}
                                    </label>
                                </div>
                            ...
                            <div className="space-y-3 p-4 bg-slate-900/40 rounded-2xl border border-slate-800">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Brand Colors</label>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <span className="text-[9px] text-slate-400 font-medium">Primary</span>
                                        <div className="flex items-center gap-2">
                                            <input type="color" value={settings.brandColors.primary} onChange={(e) => handleColorChange('primary', e.target.value)} className="w-6 h-6 rounded-lg bg-transparent border-none cursor-pointer" />
                                            <input type="text" value={settings.brandColors.primary} onChange={(e) => handleColorChange('primary', e.target.value)} className="bg-transparent text-[10px] font-mono text-slate-300 w-16 outline-none" />
                                        </div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <span className="text-[9px] text-slate-400 font-medium">Secondary</span>
                                        <div className="flex items-center gap-2">
                                            <input type="color" value={settings.brandColors.secondary} onChange={(e) => handleColorChange('secondary', e.target.value)} className="w-6 h-6 rounded-lg bg-transparent border-none cursor-pointer" />
                                            <input type="text" value={settings.brandColors.secondary} onChange={(e) => handleColorChange('secondary', e.target.value)} className="bg-transparent text-[10px] font-mono text-slate-300 w-16 outline-none" />
                                        </div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <span className="text-[9px] text-slate-400 font-medium">Text</span>
                                        <div className="flex items-center gap-2">
                                            <input type="color" value={settings.brandColors.text} onChange={(e) => handleColorChange('text', e.target.value)} className="w-6 h-6 rounded-lg bg-transparent border-none cursor-pointer" />
                                            <input type="text" value={settings.brandColors.text} onChange={(e) => handleColorChange('text', e.target.value)} className="bg-transparent text-[10px] font-mono text-slate-300 w-16 outline-none" />
                                        </div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <span className="text-[9px] text-slate-400 font-medium">Accent</span>
                                        <div className="flex items-center gap-2">
                                            <input type="color" value={settings.brandColors.accent} onChange={(e) => handleColorChange('accent', e.target.value)} className="w-6 h-6 rounded-lg bg-transparent border-none cursor-pointer" />
                                            <input type="text" value={settings.brandColors.accent} onChange={(e) => handleColorChange('accent', e.target.value)} className="bg-transparent text-[10px] font-mono text-slate-300 w-16 outline-none" />
                                        </div>
                                    </div>
                                </div>
                            </div>
